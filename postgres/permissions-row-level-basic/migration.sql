...
-- Delete the `_rowpermissions` schema from the expanded page on the left
-- or you may run: `DROP SCHEMA IF EXISTS _rowpermissions` from the SQL tab to the left
--
-- Please be careful before running, as all data in the schema will be removed

-- Create Schema
CREATE SCHEMA _rowpermissions;

-- Create Tables
CREATE TABLE _rowpermissions.user (
    id serial PRIMARY KEY,
    name text NOT NULL
);

CREATE TABLE _rowpermissions.chat (
    id serial PRIMARY KEY,
    created_at timestamp NOT NULL DEFAULT now(),
    message text NOT NULL,
    sender_id int REFERENCES _rowpermissions.user(id) NOT NULL,
    receiver_id int REFERENCES _rowpermissions.user(id) NOT NULL
);

-- Insert Seed Data
INSERT INTO "_rowpermissions"."user" ("name") VALUES
('Coleman Spickett'),
('Gallard Dreye'),
('Alysa Beecker'),
('Kellie Owbridge'),
...
INSERT INTO "_rowpermissions"."chat" ("sender_id", "receiver_id", "message") VALUES

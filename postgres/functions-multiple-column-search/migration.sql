...
-- Delete the `_multicolumn` schema from the expanded page on the left
-- or you may run: `DROP SCHEMA IF EXISTS _multicolumn` from the SQL tab to the left
--
-- Please be careful before running, as all data in the schema will be removed

-- Create Schema
CREATE SCHEMA _multicolumn;

-- Create Tables
CREATE TABLE _multicolumn.directory (
...
CREATE FUNCTION _multicolumn.search(search text)
RETURNS SETOF _multicolumn.directory AS $$
    SELECT *
    FROM _multicolumn.directory
    WHERE
...
-- Add Seed Data
INSERT INTO "_multicolumn"."directory" ("department", "name", "company") VALUES
